    ...
    with open(filename) as fp:
        ...
        for line in fp:
            if line.find('#')!=0:
                ...
                facetNameList = line[1:].strip().split()
                i+=1
            elif i == 1:
                facetValueList = line[1:].strip().split()
                i+=1
        assert(len(facetNameList) == len(facetValueList))
    return lclist, dict((facetNameList[i],facetValueList[i]) for i in range(len(facetNameList)))
...
class LightCurve:
    ...
    @property
    def timeseries(self):
        return (self._time, self._amplitude)
